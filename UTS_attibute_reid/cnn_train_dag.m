...
% net.params(net.getParamIndex('lof')).trainMethod = 'rmsprop';
% net.params(net.getParamIndex('lob')).trainMethod = 'rmsprop';
else
  state = [] ;
end
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    %%------------------lean
    if(isequal(class(net.layers(end).block),'Lean'))
        net.layers(end).block.leanRate = min(0.3,0.1*1.1^epoch);
    end
    %%------------------
...
for t=1:params.batchSize:numel(subset)
  diary('F:\jinhao\ReID\Code\reID_attri\Diary.txt'); %The file path and file name.
  diary on;
  ...
  for s=1:params.numSubBatches
    ...
    %start stn layer
    if ~isempty(params.start_your_dancing)
        if(params.epoch>params.start_your_dancing)
            net.params(net.getParamIndex('lof')).learningRate = 1e-4;
            net.params(net.getParamIndex('lob')).learningRate = 1e-4;
        end
    end
